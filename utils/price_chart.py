...
import numpy as np
# from streamlit.components.v1 import html # Not directly used in these functions
# No explicit import of altair.core or altair.schema is needed at the top,
# as we will use alt.schema directly.

# --- Custom sort order for floors (from PriceChart.py) ---
# REQUIRED_COLUMNS = [
#     'customer_name', 'floor', 'period', 'sqr',
#     'rental_VND', 'service_VND', 'foreign_ex',
...
def calculate_metrics_values(df_input):
    ...
    avg_exchange_rate = df_input['foreign_ex'].mean() if 'foreign_ex' in df_input.columns else 0
    avg_exchange_rate = 0 if pd.isna(avg_exchange_rate) else avg_exchange_rate

    return highest_rental_price,highest_service_price, avg_rental, lowest_rental_price,lowest_service_price, avg_service, avg_exchange_rate
